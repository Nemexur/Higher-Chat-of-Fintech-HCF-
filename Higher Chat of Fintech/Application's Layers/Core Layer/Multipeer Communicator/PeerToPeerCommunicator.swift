...
class MultipeerCommunicator: NSObject, IMultipeerCommunicator, MCSessionDelegate, MCNearbyServiceAdvertiserDelegate, MCNearbyServiceBrowserDelegate, MCBrowserViewControllerDelegate {
    ...
    private func getProfileUserName() {
        let context = coreDataStack?.privateNewDataManagedObjectContext
        guard let model = coreDataStack?.managedObjectModel
            else {
        ...
        do {
            if let results = try context?.fetch(fetchRequest) {
                if results.count > 1 {
                    fatalError("Multiple AppUsers")
                } else {
                    userName = results.first?.currentUser?.userName
                }
            } else {
